...
function replaceListElements(listEle, listItem){
	listEle.innerText = '';
	for (var i = 0; i < listItem.length; i++) {
		let liEle = document.createElement("li");
		liEle.appendChild(document.createTextNode(listItem[i]))
...
function  shuffleButtonClicked(e){
	replaceListElements (todolistEle , getNewShuffledArray (todolistArray));
}
// Update the todo list view with initial list of items
replaceListElements(todolistEle,todolistArray);